...
function generateCoverageComment(coverage, title, showFiles, threshold) {
    ...
    // Overall status
    const avgCoverage = (summary.statements +
        summary.branches +
        summary.functions +
        summary.lines) /
        4;
    const overallEmoji = getCoverageEmoji(avgCoverage, threshold);
    comment += `**Overall Coverage: ${formatPercentage(avgCoverage)} ${overallEmoji}**\n\n`;
    if (showFiles && files.length > 0) {
        ...
        files.forEach((file) => {
            const filePath = file.file
                .replace(process.cwd(), "")
                .replace(/^[\/\\]/, "");
            comment += `| \`${filePath}\` | ${formatPercentage(file.statements)} | ${formatPercentage(file.branches)} | ${formatPercentage(file.functions)} | ${formatPercentage(file.lines)} |\n`;
        });
    }
    comment +=
...
async function run() {
    try {
        ...
        const title = core.getInput("title", { required: false }) || "📊 Coverage Report";
        const showFiles = core.getInput("show-files", { required: false }) === "true";
        const threshold = parseInt(core.getInput("coverage-threshold", { required: false }) || "80", 10);
        if (!token) {
        ...
        if (!fs.existsSync(coverageFilePath)) {
            core.setFailed(`coverage-summary.json file not found at path ${coverageFilePath}. Did you forget to add the reporter in your vitest.config.js?`);
            return;
        }
        const coverageData = JSON.parse(fs.readFileSync(coverageFilePath, "utf8"));
        // Generate comment
        const comment = generateCoverageComment(coverageData, title, showFiles, threshold);
        ...
        const existingComment = comments.find((comment) => comment.user?.type === "Bot" &&
            comment.body?.includes("Generated by [@glideapps/vitest-coverage-tools]"));
        if (existingComment) {
